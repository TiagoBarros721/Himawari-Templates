...
import com.com.com.Engine.Gfx.Debugging.type;
import com.com.com.Engine.Utils.GameMaths;
import com.com.com.Engine.Utils.Geom.Circle;
...
public class Widget {
    ...
    public static void drawRoundRectangle(Rectangle rect, int roundness, Graphics2D g) {
        ...
        if (show) {
            ...
            g.setColor(color);
            if (drawType == type.HOLLOW)
                g.drawRoundRect((int) rect.x, (int) rect.y, (int) rect.width, (int) rect.height, roundness, roundness);
            else
                g.fillRoundRect((int) rect.x, (int) rect.y, (int) rect.width, (int) rect.height, roundness, roundness);
        }
    }
    ...
    public static void drawCircle(Circle circle, Graphics2D g) {
        ...
        if (show) {
            ...
            if (drawType == type.HOLLOW)
                ...
                g.fillOval((int) circle.x, (int) circle.y, (int) circle.radius, (int) circle.radius);
        }
    }
    ...
    public static void drawGrid(Vec2 start, Vec2 end, int tileWidth, int tileHeight, Graphics2D g) {
        ...
        if (show) {
            ...
            for (int i = 0; i < ((end.x + 1) - start.x); i += tileWidth) {
                ...
                g.drawLine((int) i, (int) start.y, i,
                        (int) (start.y + (tileHeight * ((end.y - start.y) / tileHeight))));
            }
            for (int i = 0; i < ((end.x + 1) - start.x); i += tileHeight) {
    ...
    public static void drawSpriteExt(Sprite sprite, Vec2 position, Vec2 scale, Double angle, Graphics2D g) {
        ...
        if (show) {
            ...
            img = ImageUtil.toBufferedImage(ImageUtil.resizeImage((int) (img.getWidth() * scale.x),
                    (int) (img.getHeight() * scale.y), ImageRenderer.scaleAlgorithm.SMOOTH, img));
    ...
    public static void drawHealthBar(Vec2 start, Vec2 dimensions, float amount, float maxAmount, Color background,
            Color borderColor, Color minColor, Color maxColor, Direction direction, boolean border, Graphics2D g) {
        ...
        if (show) {
            ...
            switch (direction) {
                ...
                case RIGHT:
                    ...
                    g.fillRect((int) start.x, (int) start.y, (int) (dimensions.x * (filledPercentage / 100)),
                            (int) dimensions.y);
                    break;
                case DOWN:

                    g.fillRect((int) start.x, (int) start.x + (int) (dimensions.y * (1 - filledPercentage / 100)),
                            (int) (dimensions.x),
                            (int) (dimensions.y) - (int) (dimensions.y * (1 - filledPercentage / 100)));
                    break;
                case LEFT:

                    g.fillRect((int) start.x + (int) (dimensions.x * (1 - filledPercentage / 100)), (int) start.y,
                            (int) (dimensions.x) - (int) (dimensions.x * (1 - filledPercentage / 100)),
                            (int) dimensions.y);
                    break;
                case UP:

                    g.fillRect((int) start.x, (int) start.y, (int) dimensions.x,
                            (int) (dimensions.y * (filledPercentage / 100)));
                    break;
                default:
                    break;
            }
