...
public class Path implements Serializable {
    ...
    public Path(Vec2[] knots) {
        this.knots = new ArrayList<Vec2>(Arrays.asList(knots));
        myPathPoint = knots[0];
    }

    public Path() {
        this.knots = new ArrayList<Vec2>();
    }

    public void redefineKnots(Vec2[] knots) {
        this.knots = new ArrayList<Vec2>(Arrays.asList(knots));
    }

    public List<Vec2> getKnots() {
        return knots;
    }
    ...
    public Vec2 getKnot(int index) {
        ...
        if (index >= knots.size() || index < 0)
            return null;
        return knots.get(index);
    }

    public void appendEndKnot(Vec2 knot) {
        knots.add(knot);
    }

    public void appendEndVertices(Vec2[] knot) {
        knots.addAll(Arrays.asList(knot));
    }

    public void appendStartKnot(Vec2 knot) {
        knots.add(0, knot);
    }

    public void appendStartVertices(Vec2[] knot) {
        knots.addAll(0, Arrays.asList(knot));
    }

    public void addPathEndListener(PathEndListener listener) {
        pathListener = listener;
    }

    public void removePathEndListener() {
        pathListener = null;
    }
    ...
    public void followPath(Transform t) {
        ...
        if (t.position.subtractWith(knots.get(myPointIndex)).thisMagnitude() >= segmentSize) {
            ...
            segmentSize = knots.get(myPointIndex).subtractWith(knots.get(myPointIndex + 1)).thisMagnitude();
            if (pathListener != null)
                pathListener.pathMoved(myPathPoint);
        }
    }
    ...
    public float getRelativeDistanceTraveled() {
    ...
    public void mergePaths(Path newPath, MergePathMethod method) {
        ...
        switch (method) {
            ...
            case ORDER:
                ...
                break;
            case SUFFIX:
                knots.addAll(0, newPath.getKnots());
                break;
            case JOINGREATER:
            ...
            case INTERCEPTION:
                ...
                knots = knots.stream().filter(two -> newPath.getKnots().stream().anyMatch(one -> one.equals(two)))
                        .collect(Collectors.toList());
                break;
            default:
                mergePaths(newPath, MergePathMethod.APPEND);
                break;
        }
