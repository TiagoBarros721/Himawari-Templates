...
import com.com.com.Engine.Gfx.ImageUtil;
...
public class RoomHandler {
        ...
        if (hasRooms() && currentRoom != null) {
            ...
            if (currentRoom.hasBackground()) {
                ...
                if (displaySettings == null && !isStretched()) {
                    ...
                    // Stretch = true
                    currentRoom.background = new Sprite(ImageUtil.toBufferedImage(ImageUtil.resizeImage(Window.width,
                            Window.height, scaleAlgorithm.SMOOTH, currentRoom.background.sprite)));
                        ...
                        if (tileNumberW > 1 || tileNumberH > 1) {
                            for (int i = 0; i < tileNumberW; i++) {
                                for (int j = 0; j < tileNumberH; j++)
                                    g.drawImage(currentRoom.background.sprite, i * currentRoom.background.width,
                                            j * currentRoom.background.height, null);
                            }
                        }
                    }
                }
            ...
            if (Camera.getInstance() == null) {
                ...
                /**
                 * To save a lot of memory, we need to just draw the camera viewport and not the
                 * entire map
                 * 
                 * 
                 ...
                 * (int)(i * currentRoom.tileset.width + (Camera.getOffset().x -
                 * Camera.position.position.x)),
                 * (int)(j * currentRoom.tileset.height + (Camera.getOffset().y -
                ...
                // System.out.println(Camera.ViewPort.y + (int) Camera.getViewPortOffset().y +
                // viewportOffset.x);
                int initialX = (int) Camera.getViewPortOffset().x;
                int initialY = (int) Camera.getViewPortOffset().y;

                Vec2 tileOffset = Camera.getOffset().divide(
                        new Vec2(Window.width / currentRoom.tileset.width, Window.height / currentRoom.tileset.height))
                        .round();

                for (int i = (int) Camera.getViewPortOffset().x; i < (initialX + tileOffset.x
                        + Window.width / currentRoom.tileset.width); i++) {
                    for (int j = (int) Camera.getViewPortOffset().y; j < (initialY + tileOffset.x
                            + Window.width / currentRoom.tileset.width); j++) {
                        ...
                        g.drawImage(currentRoom.tileset.getFrame(currentRoom.roomData.getTile(j, i)),
                                (int) ((j * currentRoom.tileset.width - Camera.position.position.x
                                        + Camera.getOffset().x) * Camera.viewport.x),
                                (int) ((i * currentRoom.tileset.height - Camera.position.position.y
                                        + Camera.getOffset().y) * Camera.viewport.y),
    ...
    public static boolean gotoRoom(Room newRoom) {
        ...
        if (saveBlock) {
            ...
            System.out.println("[WARNING] Room loading is self-repairing, wait " + (System.currentTimeMillis() - start)
                    + " more milliseconds");
            return false;
        }
