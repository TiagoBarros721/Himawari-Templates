...
public class Debugging {
    ...
    public static void drawDebugRectangle(Rectangle rect, Graphics2D g) {
        ...
        if (showDebug) {
            ...
            if (drawType == type.HOLLOW)
                g.drawRect((int) point.x, (int) point.y, (int) rect.width, (int) rect.height);
            else
                g.fillRect((int) point.x, (int) point.y, (int) rect.width, (int) rect.height);
        }
    }
    ...
    public static void drawDebugCircle(Circle circle, Graphics2D g) {
        ...
        if (showDebug) {
            ...
            if (drawType == type.HOLLOW)
                g.drawOval((int) point.x, (int) point.y, (int) circle.radius, (int) circle.radius);
            else
                g.fillOval((int) point.x, (int) point.y, (int) circle.radius, (int) circle.radius);
        }
    }
    ...
    public static void drawRay(RayHit hit, Graphics2D g) {
        ...
        if (showDebug) {
            ...
            g.setColor(debugColor);
            Vec2 point1 = Camera
                    .calculateWindowTowindowPoint(new Vec2((int) -hit.getOrigin().x, (int) -hit.getOrigin().y));
    ...
    public static void drawDebugGrid(Vec2 start, Vec2 end, int tileWidth, int tileHeight, Graphics2D g) {
        ...
        if (showDebug) {
            ...
            for (int i = 0; i < ((point2.x + 1) - point1.x); i += tileWidth) {
                ...
                g.drawLine((int) i, (int) point1.y, i,
                        (int) (point1.y + (tileHeight * ((point2.y - point1.y) / tileHeight))));
            }
            for (int i = 0; i < ((end.x + 1) - point1.x); i += tileHeight) {

                g.drawLine((int) point1.x, i,
                        (int) (int) (point1.x + (tileWidth * ((point2.x - point1.x) / tileWidth))), i);
            }
        }
    }
