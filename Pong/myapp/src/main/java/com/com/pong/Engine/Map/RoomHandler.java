    ...
    public static void render(Graphics2D g) {
        ...
        if (hasRooms()) {
            ...
            if (Camera.getInstance() == null) {
                ...
                // viewportOffset.x);
                for (int i = (int) Camera.getViewPortOffset().x; i < Camera.ViewPort.y
                        + (int) Camera.getViewPortOffset().x + viewportOffset.y; i++) {
                    for (int j = (int) Camera.getViewPortOffset().y; j < Camera.ViewPort.x
                            + (int) Camera.getViewPortOffset().y + viewportOffset.x; j++) {
                        ...
                        g.drawImage(currentRoom.tileset.getFrame(currentRoom.roomData.getTile(j, i)),
                                (int) (j * currentRoom.tileset.width - Camera.position.position.x
                                        + Camera.getOffset().x) * Camera.getSize(),
                                (int) (i * currentRoom.tileset.height - Camera.position.position.y
                                        + Camera.getOffset().y) * Camera.getSize(),
