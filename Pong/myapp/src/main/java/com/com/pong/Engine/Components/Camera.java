...
public class Camera extends Component {
    ...
    public static void setSize(int size) {
        ...
        if (size <= 0) {
            System.out.println("[ERROR] Invalid camera size");
            return;
        }
        Camera.size = size;
    ...
    public static Vec2 getViewPortOffset() {
        ...
        Vec2 viewPos = new Vec2(
                GameMaths.clamp((position.position.y - offset.y) / cRoomTileSizeY, 0,
                        RoomHandler.currentRoom.roomData.getHeight()),
                GameMaths.clamp((position.position.x - offset.x) / cRoomTileSizeX, 0,
                        RoomHandler.currentRoom.roomData.getWidth()));
